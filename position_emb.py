...
def precompute_theta_pos_frequencies(
    ...
    """
    Returns: 
        freqs_complex: (seq_len, head_dim / 2), frequencies in complex form
    
    Computes the frequency theta during the initialization of Transformer.
    It'll be send into to apply_rotery_pe during inference.
    """
    assert head_dim % 2 == 0
    k = torch.arange(0, head_dim, 2).float() # (dim/2,), [0, 2, ..., head_dim/2]
    theta = 1.0 / (base ** (k / head_dim))
    
    # Position for each token in the sequence
    pos = torch.arange(seq_len) # [0, 1, 2, ... seq_len]
    
    # pos * theta or m * theta
    freqs = pos.outer(theta) # (seq_len, dim/2)
...
def apply_rotary_embeddings(
    ...
    Returns:
        x: (b, seq_len, dim)
    """
    # (B, seq_len, dim) -> (B, seq_len, H, head_dim / 2, 2) -> (B, seq_len, H, head_dim / 2)
    batch_size, seq_len, H, head_dim = x.shape
    x_complex = torch.view_as_complex( x.reshape(*x.shape[:-1], head_dim // 2, 2) )
    
    # (seq_len, head_dim / 2) -> (B, seq_len, H, head_dim / 2)
    freqs_complex = freqs_complex.unsqueeze(0).unsqueeze(2)
    
    # elementwise multiplication, not mutmul
    x_rotery = x_complex * freqs_complex # (B, seq_len, H, head_dim / 2)
    
    # (B, seq_len, H, head_dim / 2) -> (B, seq_len, H, head_dim / 2, 2) -> (B, seq_len, H, head_dim)
    x_out = torch.view_as_real(x_rotery).reshape(*x.shape)
